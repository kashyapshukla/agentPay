...
If you're a developer building an autonomous AI system—for instance, a LangChain agent that researches the web or an AutoGen multi-agent squad—you eventually hit a wall: **How do your agents pay for the external APIs they need?**

Here is exactly how you integrate AgentPay into your project from start to finish.
...
You ping your local AgentPay API (or your cloud instance) to create an isolated key specifically for this single agent script:
```bash
...
```bash
curl -X POST http://localhost:8000/v1/billing/wallets/0a2a60c2-.../topup \
-H "Content-Type: application/json" \
-d '{"amount": 5.0}'
```
...
When the agent decides to execute a task using a paid API, it invokes the `AgentPayMeterTool`. This tool securely streams a metering event to your backend.
